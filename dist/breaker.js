...
var breaker = function breaker(chain) {
  ...
  if (Array.isArray(chain) && chain.length >= 5) {
    ...
    do {
      ...
      maxPIndex = maxQIndex - 2; //check if chain[maxQIndex] < chain[maxQIndex + 1]
      ...
      if (maxQIndex === N - 2 || chain[maxQIndex] < chain[maxQIndex + 1] && maxQIndex + 1 <= N - 2) {
        ...
        for (var i = minPIndex; i <= maxPIndex; i++) {
          var cost = chain[i] + chain[maxQIndex]; //check if new sum is smaller than previous and save it
      ...
      maxQIndex--;
    } while (maxPIndex >= minPIndex && maxQIndex >= minQIndex); //keep looping until maxPIndex and maxQIndex are not reached
